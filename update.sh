...
#      echo "kubectl -n ${PLUGIN_NAMESPACE} set image deployment/${DEPLOY} ${CONTAINER}=${PLUGIN_REPO}:${PLUGIN_TAG}FORCE"
      kubectl -n ${PLUGIN_NAMESPACE} set image deployment/${DEPLOY} \
        ${CONTAINER}=${PLUGIN_REPO}:${PLUGIN_TAG}FORCE
    fi
#    echo "kubectl -n ${PLUGIN_NAMESPACE} set image deployment/${DEPLOY} ${CONTAINER}=${PLUGIN_REPO}:${PLUGIN_TAG} --record"
    kubectl -n ${PLUGIN_NAMESPACE} set image deployment/${DEPLOY} \
